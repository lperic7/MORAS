...
Proof.
  intros. simpl. assert(H1: forall x y:list B, AndL(NotL x)(NotL y) = NotL(OrL x y)).
  - induction x0, y0; simpl; try reflexivity.
    + simpl in IHx0. specialize IHx0 with y0. rewrite IHx0. induction a; simpl; reflexivity.
  - specialize H1 with x y. rewrite H1. assert (H2: forall z :list B, NotL(NotL z) = z).
    + intros. induction z.
      * simpl. reflexivity.
      * simpl. rewrite IHz. induction a; simpl; reflexivity.
    + specialize H2 with (OrL x y). rewrite H2. reflexivity.
Qed.

Lemma ALU_One (n : nat) (x y : list B) :
  length x = n /\ length y = n /\ n <> 0 -> ALU x y I I I I I I = repeat O (pred n) ++ [I].
Proof.
  intros Hx. destruct Hx as [Hx Hy]. destruct Hy as [Hy Hn]. simpl.
  rewrite Hx, Hy. destruct n.
